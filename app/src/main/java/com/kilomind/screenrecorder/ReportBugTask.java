...
import static com.kilomind.screenrecorder.Tracker.*;
...
public class ReportBugTask extends AsyncTask<Void, Void, Integer> {
    ...
    protected void onPostExecute(Integer exitValue) {
        ...
        emailIntent.putExtra(Intent.EXTRA_EMAIL, new String[]{"scr.screen.recorder@hotmail.com"});
        String version = Utils.getAppVersionName(context);
        String subject = context.getString(R.string.error_report_subject) + " " + context.getString(R.string.free_app_name) + " " + version + " - " + errorCode + " - " + Build.DEVICE + " - " + Build.VERSION.RELEASE ;
        emailIntent.putExtra(Intent.EXTRA_SUBJECT, subject);
        emailIntent.putExtra(Intent.EXTRA_TEXT, context.getString(R.string.error_report_text));
