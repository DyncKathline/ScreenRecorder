...
public class ScaleGestureDetector {
    ...
    private float getRawX(MotionEvent event, int pointer) {
        float offset = event.getRawX() - event.getX();
        return event.getX(pointer) + offset;
    }

    private float getRawY(MotionEvent event, int pointer) {
        float offset = event.getRawY() - event.getY();
        return event.getY(pointer) + offset;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (mDoubleTapMode == DOUBLE_TAP_MODE_IN_PROGRESS) {
            ...
            // gesture started
            focusX = mDoubleTapEvent.getRawX();
            focusY = mDoubleTapEvent.getRawY();
            if (event.getRawY() < focusY) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                sumX += getRawX(event, i);
                sumY += getRawY(event, i);
            }
        ...
        for (int i = 0; i < count; i++) {
            ...
            devSumX += Math.abs(getRawX(event, i) - focusX) + touchSize;
            devSumY += Math.abs(getRawY(event, i) - focusY) + touchSize;
        }
