...
import static com.kilomind.screenrecorder.Tracker.ERROR;
import static com.kilomind.screenrecorder.Tracker.ERROR_;
import static com.kilomind.screenrecorder.Tracker.RECORDING_ERROR;
import static com.kilomind.screenrecorder.Tracker.STARTUP_ERROR;
...
public class ErrorDialogHelper implements IRecordingProcess.RecordingProcessObserver {
    ...
    public void onStateChange(IRecordingProcess process, RecordingProcessState state, RecordingInfo recordingInfo) {
        ...
        switch (state) {
            ...
            case CPU_NOT_SUPPORTED_ERROR:
                ...
                message = getString(R.string.cpu_error_message, Build.CPU_ABI, getString(R.string.free_app_name));
                title = getString(R.string.cpu_error_title);
                report = false;
                break;

            case INSTALLATION_ERROR:
                message = getString(R.string.installation_error_message, getString(R.string.free_app_name));
    ...
    private void showSuError() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            String message = getString(R.string.su_required_lollipop_message, getString(R.string.free_app_name), getString(R.string.free_app_no_root_name));
            if (suIntent != null) {
                message += " " + getString(R.string.su_required_lollipop_denied_message, suName, getString(R.string.free_app_name));
            ...
            String message = getString(R.string.su_denied_message, suName, getString(R.string.free_app_name));
            intent.putExtra(DialogActivity.MESSAGE_EXTRA, message);
            intent.putExtra(DialogActivity.POSITIVE_INTENT_EXTRA, suIntent);
            intent.putExtra(DialogActivity.POSITIVE_EXTRA, suName);
        }
